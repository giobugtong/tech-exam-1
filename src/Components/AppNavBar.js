...
export default function AppNavBar() {
  ...
  return <>
    <Navbar variant="dark" bg="dark" expand="lg" expanded={expandNavBar}>
        <Container fluid>
            <Navbar.Brand as={NavLink} to="/"><span style={{ fontSize: "clamp(1rem, 5vw, 1.5rem)" }}>eWave React Assessment</span></Navbar.Brand>
            ...
            <Nav className="me-auto">
                ...
                <NavDropdown title="Tasks" id="basic-nav-dropdown">
                {
                    dropDownItems.map((item, index) => {
                        return <NavDropdown.Item key={index} as={Link} to={`task-${index + 1}`}>{item}</NavDropdown.Item>
                    })
                }
                </NavDropdown>
            </Nav>
